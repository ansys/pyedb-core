...
class ServerBackend(PolygonBackend):
    ...
    def remove_self_intersections(
        ...
        """
        return self._stub.RemoveSelfIntersections(
            messages.polygon_data_with_tol_message(polygon, tol)
        )
    ...
    def move(self, polygon: PolygonData, vector: tuple[float, float]) -> PolygonData:
        ...
        return self._stub.Transform(
            messages.polygon_data_transform_message("move", polygon, vector)
        )
    ...
    def scale(
        ...
        """
        return self._stub.Transform(
            messages.polygon_data_transform_message("scale", polygon, factor, center)
        )
    ...
    def closest_point(
        ...
        """
        return self._stub.GetClosestPoints(
            messages.polygon_data_with_points_message(polygon, point=point)
        ).points[0]
    ...
    def closest_points(
        ...
        """
        result = self._stub.GetClosestPoints(
            messages.polygon_data_with_points_message(polygon1, polygon=polygon2)
        ).points
