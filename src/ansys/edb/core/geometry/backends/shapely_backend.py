...
class ShapelyBackend(PolygonBackend):
    ...
    @staticmethod
    def _remove_consecutive_duplicate_points(points: list[PointData], tol: float = 1e-9) -> list[PointData]:
        ...
        for pt in points[1:]:
            ...
            if not (math.isclose(pt.x.double, last_pt.x.double, rel_tol=tol) and math.isclose(pt.y.double, last_pt.y.double, rel_tol=tol)):
    ...
    def is_box(self, polygon: PolygonData, tol: float = 1e-9) -> bool:
        ...
        if polygon.has_holes() or polygon.has_arcs(): # If the polygon was set up with arcs of zero height, the has_arcs() will return False.
            return False
        
        points = ShapelyBackend._sanitize_points(polygon.points.copy())
        points = [points[-2], points[-1], *points, points[0], points[1]]  # Close the loop on both ends
        ...
        while index < len(points):
            ...
            if not (math.isclose(dot_product, 0.0, rel_tol=tol) or math.isclose(dot_product, 1.0, rel_tol=tol)):
                return False
