...
class HFSSPISimulationPreference(Enum):
    """Provides an enum representing HFSS PI simulation preferences."""
...
class HFSSPIModelType(Enum):
    """Provides an enum representing HFSS PI model types."""
...
class SurfaceRoughnessModel(Enum):
    """Provides an enum representing HFSS PI surface roughness models."""
...
class ImprovedLossModel(Enum):
    """Provides an enum representing HFSS PI improved loss models."""
...
class ConductorModeling(Enum):
    """Provides an enum representing HFSS PI conductor modeling options."""
...
class ErrorTolerance(Enum):
    """Provides an enum representing HFSS PI error tolerance values."""
...
class HFSSPIGeneralSettings(SimulationSettingsBase):
    ...
    def min_plane_area_to_mesh(self) -> Value:
        """:class:`.Value`: (General Mode Only) The minimum plane area to mesh. \
            Ignore geometry smaller than this value."""
        return Value(self.__stub.GetMinPlaneAreaToMesh(self.msg))

    @min_plane_area_to_mesh.setter
    def min_plane_area_to_mesh(self, min_plane_area_to_mesh: Value):
        self.__stub.SetMinPlaneAreaToMesh(
            messages.value_property_message(self, messages.value_message(min_plane_area_to_mesh))
        )

    @property
    def min_void_area_to_mesh(self) -> Value:
        """:class:`.Value`: (General Mode Only) The minimum void area to mesh. Ignore voids smaller than this value."""
        return Value(self.__stub.GetMinVoidAreaToMesh(self.msg))

    @min_void_area_to_mesh.setter
    def min_void_area_to_mesh(self, min_void_area_to_mesh: Value):
        self.__stub.SetMinVoidAreaToMesh(
            messages.value_property_message(self, messages.value_message(min_void_area_to_mesh))
        )

    @property
    def snap_length_threshold(self) -> Value:
        """:class:`.Value`: (General Mode Only) The snap length threshold. \
            Snap vertices separated by less than this value."""
        return Value(self.__stub.GetSnapLengthThreshold(self.msg))

    @snap_length_threshold.setter
    def snap_length_threshold(self, snap_length_threshold: Value):
        self.__stub.SetSnapLengthThreshold(
            messages.value_property_message(self, snap_length_threshold)
        )
    ...
    def rms_surface_roughness(self) -> Value:
        """:class:`.Value`: RMS surface roughness."""
        return Value(self.__stub.GetRMSSurfaceRoughness(self.msg))

    @rms_surface_roughness.setter
    def rms_surface_roughness(self, rms_surface_roughness: Value):
        self.__stub.SetRMSSurfaceRoughness(
            messages.value_property_message(self, rms_surface_roughness)
        )
...
class HFSSPINetProcessingSettings(SimulationSettingsBase):
    ...
    def include_nets(self) -> List[str]:
        """:obj:`list` of :obj:`str`: Nets to include in HFSS PI simulation."""
...
class HFSSPIPowerGroundNetsSettings(SimulationSettingsBase):
    ...
    def ignore_small_holes_min_diameter(self) -> Value:
        """:class:`.Value`: Diameter that holes smaller than the diameter will be ignored."""
        return Value(self.__stub.GetIgnoreSmallHolesMinDiameter(self.msg))

    @ignore_small_holes_min_diameter.setter
    def ignore_small_holes_min_diameter(self, ignore_small_holes_min_diameter: Value):
        self.__stub.SetIgnoreSmallHolesMinDiameter(
            messages.value_property_message(self, ignore_small_holes_min_diameter)
        )
