...
import ansys.edb.core.geometry.arc_data as arc_data
...
test_arcs = [
    ArcData((1, 1), (2, 2)),
    ArcData((1, 1), (1 - 1e-10, 1 + 1e-10)),
    ArcData((1, 1), (1, 1), height=1e9),
    ArcData((1, 1), (1 - 1e-8, 1 + 1e-8)),
]
...
def test_center(mocked_stub):
    mock = mocked_stub(arc_data, ArcData).GetCenter
    ...
    arc = ArcData((2, 0), (0, 2), height=2.0)
    center = arc.center
    mock.assert_called_once()
    assert center.x == 2
    assert center.y == 2
...
def test_bbox(mocked_stub):
    mock = mocked_stub(arc_data, ArcData)
    ...
    bbox = arc.bbox
    assert len(bbox) == 4
    assert bbox.points == PolygonData(lower_left=(0, 0), upper_right=(3, 4)).points
...
def test_is_big(height, is_big):
    arc = ArcData(start=(0, 0), end=(3, 4), height=height)
    assert Value(arc.start.distance(arc.end)).equals(5, 1e-6)
...
@pytest.mark.parametrize(
    ...
    [
        [False, 0, ArcData(start=(0, 0), end=(3, 4), height=0), 5],
        [
            True,
            1,
            ArcData(start=(0, 0), end=(3, 4), radius=1, direction="cw", is_big=False),
            math.acos(1) / 4,
        ],
    ],
)
def test_length(mocked_stub, expect_call, height, arc, length):
    ...
    assert Value(arc.length).equals(length, 1e-6)
    if expect_call:
...
def test_angle(mocked_stub):
    ...
    arc = ArcData(start=(0, 0), end=(3, 4))

    assert arc.angle() == 0.5
    mock.GetAngle.assert_called_once()

    arc1 = ArcData(start=(0, 0), end=(3, 4))
    arc2 = ArcData(start=(0, 0), end=(-3, -4))
    arc1.angle(arc2)
...
@pytest.mark.parametrize(
    "arc, tangent",
    [
        [ArcData(start=(0, 0), end=(3, 4)), (3, 4)],
        [ArcData(start=(0, 0), end=(3, 4), height=-0.1), (-4, 3)],
        [ArcData(start=(0, 0), end=(3, 4), height=+0.1), (4, -3)],
    ],
)
