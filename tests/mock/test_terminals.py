...
from ansys.edb.core.inner.messages import (
    ...
    point_term_params_message,
    point_term_set_params_message,
    port_post_processing_prop_message,
    rlc_message,
    value_message,
)
import ansys.edb.core.layer.layer as layer_api
import ansys.edb.core.terminal.terminals as terminals
...
def test_bundle_terminal_create(mocked_stub, point_terminal, bundle_terminal, edb_obj_msg):
    ...
    mock.assert_called_once_with(edb_obj_collection_message([point_terminal, bundle_terminal]))
...
def test_bundle_terminal_get_terminals(mocked_stub, bundle_terminal, term_type, term_cls):
    get_terminals = mocked_stub(terminals, BundleTerminal).GetTerminals
    get_terminals.return_value = expected = create_edb_obj_msgs(2)
    get_params = mocked_stub(terminals, Terminal).GetParams
...
def test_bundle_terminal_ungroup(mocked_stub, bundle_terminal):
    ...
    mock = mocked_stub(terminals, BundleTerminal).Ungroup
...
def test_point_terminal_create(mocked_stub, layout, net, layer, edb_obj_msg):
    mock = mocked_stub(terminals, PointTerminal).Create
    mock.return_value = edb_obj_msg

    pt = PointTerminal.create(layout, net, layer, "test-point-term", (1, 2))

    mock.assert_called_once_with(
        point_term_creation_message(layout, net, layer, "test-point-term", (1, 2))
    )
...
def test_point_terminal_get_params(mocked_stub, point_terminal, layer, lyr_type_mock):
    point = (1e-9, 2e-9)
    mock = mocked_stub(terminals, PointTerminal).GetParameters
    mock.return_value = point_term_params_message(layer, point)
...
def test_point_terminal_set_params(mocked_stub, point_terminal, layer_ref, point, success):
    mock = mocked_stub(terminals, PointTerminal).SetParameters
    mock.return_value = bool_message(success)

    point_terminal.params = (layer_ref, point)

    mock.assert_called_once_with(point_term_set_params_message(point_terminal, layer_ref, point))


def test_terminal_get_params(mocked_stub, point_terminal, bundle_terminal, layer, lyr_type_mock):
    mock = mocked_stub(terminals, Terminal)
    mock.GetParams.return_value = term_pb2.TermParamsMessage(
        ...
        name="name",
        impedance=value_message(50),
        source_amplitude=value_message(10),
        source_phase=value_message(5),
        s_param_model="model",
        rlc=rlc_message(Rlc(r=10, r_enabled=True)),
        port_post_processing_prop=port_post_processing_prop_message(
...
def test_terminal_set_params(mocked_stub, point_terminal, bundle_terminal, layer):
    mock = mocked_stub(terminals, Terminal).SetParams
    mock.return_value = None
